...
async function main() {
	...
	planetOrbit.local.translation = Vector.Vec3(state.global.planetDisplacement.value);
	...
	engine.camera.local.translation = Vector.Vec3(state.global.cameraPosition.value);
	...
	planetOrbit.updates.push((gl, dt) => {
		...
		if (planetOrbitAngle > fullRevolution)
			planetOrbitAngle = 0
		planetOrbit.local.rotation = Quaternion.FromAxisAngle(Vector.up, planetOrbitAngle)
	})
	...
	engine.mouseCallbacks.push(({ position: pos, id: data, button }) => {
		if (data.id === planetId && button === MouseButton.LEFT) {
			console.log(
				`Hit Object ${data.id} at WorldPos:`, data.x, data.y, data.z);
		...
		} else if (data.id !== planetId && data.id > 0 && button === MouseButton.RIGHT) {
			console.log(
				`Deleting Object ${data.id} at WorldPos:`, data.x, data.y, data.z);
			IDManager.get().getObject(data.id)?.destroy();
		} else {
			console.log(`${pos.x}, ${pos.y} -> ${data.x}, ${data.y}, ${data.z}, ${data.id} Hit Background`);
		}
	})
