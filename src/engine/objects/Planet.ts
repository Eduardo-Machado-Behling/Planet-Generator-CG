...
class SphereMesh {
  ...
  constructor(
      vao: WebGLVertexArrayObject, outBuffer: WebGLBuffer,
      indices: Uint16Array) {
    this.vao = vao;
...
export class Planet extends GameObject {
  ...
  async draw(gl: WebGL2RenderingContext) {
    ...
    this.program = await ShaderManager.load(gl);
  ...
  async loadModel(gl: WebGL2RenderingContext) {
    this.program = await ShaderManager.load(gl, 'lathe', 'lathe', (program) => {
      gl.transformFeedbackVaryings(
          program, ['vOutPos', 'vOutNormal', 'vOutUV'], gl.INTERLEAVED_ATTRIBS);
    });
    const circleData =
        this.generateCircleProfile(this.radius, this.rimResolution);

    const slices = this.latheResolution;

    this.sphereMesh = this.createLathedMesh(
        gl, this.program, circleData.pos, circleData.norm, circleData.v,
        slices);
  }


  private createLatheIndices(slices: number, pointsPerSlice: number):
      Uint16Array {
    ...
    for (let s = 1; s <= slices; s++) {
      for (let p = 0; p < pointsPerSlice - 1; p++) {
        ...
        const p0 = (currentSlice * pointsPerSlice) + p;        // Bottom-Left
        const p1 = (nextSlice * pointsPerSlice) + p;           // Bottom-Right
        const p2 = (currentSlice * pointsPerSlice) + (p + 1);  // Top-Left
        const p3 = (nextSlice * pointsPerSlice) + (p + 1);     // Top-Right
  ...
  private createLathedMesh(
      ...
      circlePos: Float32Array, circleNormals: Float32Array,
      circleVs: Float32Array, slices: number): SphereMesh {
...
function bindAttrib(
    gl: WebGL2RenderingContext, loc: number, buf: WebGLBuffer|null,
    size: number) {
  gl.bindBuffer(gl.ARRAY_BUFFER, buf);
