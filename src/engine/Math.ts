...
export class Vector {
	...
	dot(b: Vector): number {
		if (this.dim !== b.dim) throw new Error("Dimension mismatch in dot product");
		let sum = 0;
		for (let i = 0; i < this.dim; i++) {
	...
	static cross(a: Vector, b: Vector): Vector {
		if (a.dim !== 3 || b.dim !== 3) throw new Error("Cross product requires 3D vectors");
...
export class SquaredMatrix {
	...
	static MakeOrtho(left: number, right: number, bottom: number, top: number, near: number, far: number): SquaredMatrix {
		const m = new SquaredMatrix(4);
		const out = m.mat;
