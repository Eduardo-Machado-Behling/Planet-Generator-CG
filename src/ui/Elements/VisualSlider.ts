...
export class VisualSliderNumber implements VisualElement<number> {
	...
	constructor(
		value: number, elementId: string, min: number, max: number,
		step: number = 0.1, description: string = '') {
		this._value = value;
	...
	attach(root: HTMLElement, onGlobalChange: () => void): void {
		const rangeInput =
			root.querySelector(`#${this.elementId}-range`) as HTMLInputElement;
		const numInput =
			root.querySelector(`#${this.elementId}-num`) as HTMLInputElement;
