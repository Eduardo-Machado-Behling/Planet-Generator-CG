...
export abstract class VisualLayer implements VisualElement<NoiseLayer> {
	...
	attach(root: HTMLElement, onGlobalChange: () => void): void {
		...
		const header =
			this.containerEl.querySelector('.panel-header') as HTMLElement;
		const content =
			this.containerEl.querySelector('.panel-content') as HTMLElement;
		const removeBtn =
			this.containerEl.querySelector('.btn-remove') as HTMLElement;
		const blendSelect = this.containerEl.querySelector('.setting-blend-mode') as
			HTMLSelectElement;

		const title = this.containerEl.querySelector('.panel-title') as HTMLElement;
		const handle =
			this.containerEl.querySelector('.drag-handle') as HTMLElement;
		const enableToggle = this.containerEl.querySelector(
			'.layer-enable-toggle') as HTMLInputElement;


		
		header.addEventListener('click', (e) => {
			const t = e.target as HTMLElement;
			if (t.closest('.btn-remove') || t.closest('.drag-handle') ||
				t.closest('.setting-blend-mode') || t === title)
				return;
			content.classList.toggle('collapsed');
			content.style.display =
				content.classList.contains('collapsed') ? 'none' : 'grid';
		});
		...
		removeBtn.addEventListener('click', (e) => {
			...
			const event = new CustomEvent(
				'layer-remove', { detail: { id: this.id }, bubbles: true });
			this.containerEl?.dispatchEvent(event);
		});

		
		handle.addEventListener(
			'mousedown', () => this.containerEl!.draggable = true);
		handle.addEventListener(
			'mouseup', () => this.containerEl!.draggable = false);
		this.containerEl.addEventListener('dragstart', (e) => {
			this.containerEl!.classList.add('dragging');
			const event = new CustomEvent(
				'layer-drag-start', { detail: { el: this.containerEl }, bubbles: true });
			this.containerEl?.dispatchEvent(event);
		});
