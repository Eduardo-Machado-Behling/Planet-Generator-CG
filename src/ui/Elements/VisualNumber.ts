...
export class VisualNumber implements VisualElement<number> {
  ...
  constructor(
      value: number, elementId: string, step: number = 0.1,
      min: number|null = null, description: string = '') {
    this._value = value;
  ...
  renderHTML() {
    return `<input type="number" id="${this.elementId}" value="${
        this._value}" step="${
        this.step}" style="width: 60%; background: #121212; border: 1px solid #3e3e3e; color: white; padding: 4px; border-radius: 3px;">`;
  }
