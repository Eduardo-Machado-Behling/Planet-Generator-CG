...
export class VisualVector3 implements VisualElement<{
  x: number;
  y: number;
  z: number;
}> {
  ...
  subscribe(
    observer: ElementObserver<{ x: number; y: number; z: number }>,
  ): () => void {
  ...
  attach(root: HTMLElement, onGlobalChange: () => void): void {
    this.inputs = ["x", "y", "z"].map(
      (axis) =>
        root.querySelector(`#${this.elementId}-${axis}`) as HTMLInputElement,
    );
