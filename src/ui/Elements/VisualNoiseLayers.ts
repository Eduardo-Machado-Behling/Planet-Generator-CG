...
export class VisualLayerList implements VisualElement<NoiseLayer[]> {
	...
	private dragCache: { elements: HTMLElement[]; rects: DOMRect[]; } | null = null;

	private rafPending = false;
	private lastInsertPosition: { drag: HTMLElement, after: Element | null } | null =
		null;

	attach(root: HTMLElement, onGlobalChange: () => void): void {
		this.containerEl = root.querySelector('#' + this.elementId) as HTMLElement;
		const addBtn =
			root.querySelector('#' + this.elementId + '-btn-add') as HTMLElement;
		const typeSelect =
			root.querySelector('#' + this.elementId + '-type-select') as
			HTMLSelectElement;
		...
		this.containerEl.addEventListener(
			'dragover', this.onDragOverThrottled.bind(this));
	...
	private buildDragCache(): void {
		...
		const elements = Array.from(this.containerEl.querySelectorAll<HTMLElement>(
	...
	private setBaseLayer(id: string): void {
		...
		const oldBase =
			this._baseLayerId ? this.layerMap.get(this._baseLayerId) : null;
