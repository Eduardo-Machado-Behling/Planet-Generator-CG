...
export class GlobalSettings {
  playrate = new VisualSliderNumber(
      1.0, 'global-playrate', 0, 10, 0.01,
      'Global simulation speed multiplier');

  focusOnPlanet = new VisualToggle(
      true, 'global-scene-camera-relative',
      'Lock camera rotation to the planet');
  ...
  planetDisplacement = new VisualVector3(
      ...
      'Planet offset from the sum',
  );

  sunColor = new VisualColor(
      '#f19e04',
      'global-color',
      'Color of the direct sunlight',
  );
}

export class SphereSettings {
  vertices = new VisualNumber(
      1000, 'sphere-vertices', 1, 0, 'Mesh resolution (Fibonacci points)');

  radius = new VisualNumber(
      1.0, 'sphere-radius', 0.1, 0, 'Base radius of the planet');

  maxTrees = new VisualNumber(
      20, 'sphere-trees', 1, 0, 'Maximum number of trees allowed');

  displacementMult = new VisualSliderNumber(
      1.0, 'sphere-displacement-mult', 0, 2, 0.0001,
      'Global terrain height multiplier');

  pickingMinimum = new VisualSliderNumber(
      0.4, 'sphere-picking', -2, 5, 0.01,
      'Minimum height required to place trees');
  ...
  gradient = new VisualGradient(
      ...
      'sphere-gradient', 'Biome color gradient based on height');
}
...
export class AtmosphereSettings {
  ...
  intensity = new VisualSliderNumber(
      ...
      5,
      0.1,
      'Atmosphere brightness intensity',
  );
  ...
  scale = new VisualSliderNumber(
      ...
      2.0,
      0.01,
      'Atmosphere radius relative to planet',
  );
}
    ...
    if (dataObject === this._currentState) {
      ...
      keys.forEach((key) => {
        ...
        if (value && typeof value === 'object' && 'renderHTML' in value) {
          ...
          section.innerHTML =
              `<h3 style="font-size: 0.9rem; color: #888; text-transform: uppercase; margin-bottom: 5px;">${
                  key}</h3>`;
  ...
  private createGroupPanel(
    ...
    Object.keys(dataObject).forEach((prop) => {
      ...
      if (val && typeof val === 'object' && 'renderHTML' in val) {
        ...
        if (vEl.description) {
          labelContent += ` <span title="${
              vEl.description}" style="cursor: help; color: #666; margin-left: 4px;">ⓘ</span>`;
        }
