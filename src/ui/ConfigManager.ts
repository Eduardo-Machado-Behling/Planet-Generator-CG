...
export class GlobalSettings {
	...
	cameraRelativePlanet = new VisualToggle(false, 'global-scene-camera-relative')
	cameraPosition = new VisualVector3(
		{ x: 0, y: 0, z: -2 }, 'global-scene-camera-pos',
		'Camera position');
	planetDisplacement = new VisualVector3(
		{ x: 1, y: 0.5, z: 0 }, 'global-planet-displacement',
		'Direction vector of the light source');
	sunColor = new VisualColor(
		'#f19e04', 'global-color', 'Scattering color of the atmosphere');
...
export class SphereSettings {
	...
	displacementMult = new VisualSliderNumber(1.0, 'sphere-displacement-mult', 0, 2, 0.0001);
	pickingMinimum = new VisualSliderNumber(0.4, 'sphere-picking', -2, 5, 0.01);
	gradient = new VisualGradient(
		[
			{ offset: 0.00, color: '#000080' }, { offset: 0.45, color: '#0040FF' },
			{ offset: 0.55, color: '#D2B48C' }, { offset: 0.60, color: '#228B22' },
			{ offset: 0.75, color: '#696969' }, { offset: 0.90, color: '#FFFFFF' }
		],
		'sphere-gradient');
}
...
export class AtmosphereSettings {
	enabled = new VisualToggle(
		true, 'atm-enabled', 'Enable/Disable Atmosphere Rendering');
	color = new VisualColor(
		'#3366ff', 'atm-color', 'Scattering color of the atmosphere');
	intensity = new VisualSliderNumber(
		1.0, 'atm-intensity', 0, 5, 0.1, 'Brightness intensity');
	falloff = new VisualSliderNumber(
		4.0, 'atm-falloff', 0.1, 10, 0.1, 'Exponential falloff of density');
	scale = new VisualSliderNumber(
		1.15, 'atm-scale', 1.0, 2.0, 0.01,
...
export class ConfigManager {
		...
		if (dataObject === this._currentState) {
			...
			keys.forEach(key => {
				...
				if (value && typeof value === 'object' && 'renderHTML' in value) {
					...
					section.innerHTML =
						`<h3 style="font-size: 0.9rem; color: #888; text-transform: uppercase; margin-bottom: 5px;">${key}</h3>`;
	...
	private createGroupPanel(title: string, dataObject: any, parent: HTMLElement): void {
